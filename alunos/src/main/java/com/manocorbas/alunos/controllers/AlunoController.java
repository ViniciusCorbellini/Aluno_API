...
public class AlunoController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<Aluno> getAlunoById(@PathVariable(value = "id") Long id) throws Exception{
        Aluno aluno = alunoService.findAlunoById(id);
        return new ResponseEntity<>(aluno, HttpStatus.FOUND);
    }

    @PutMapping("/{id}")
    public ResponseEntity<Aluno> updateAlunos(@PathVariable(value = "id") Long id, @RequestBody AlunoDTO alunoDTO) throws Exception {
